...
void drawArraysWith(const ArrayPtr *arr, GLenum mode, GLint first, GLsizei count)
{
	...
	if (mode == ENUM_QUADS)
	{
		...
		// Client-side indices need the element buffer binding to be empty.
		if (S.elementBuffer != 0)
			es.BindBuffer(ENUM_ELEMENT_ARRAY_BUFFER, 0);
		...
		while (remaining > 0)
		{
			...
			es.DrawElements(ENUM_TRIANGLES, n / 4 * 6, ENUM_UNSIGNED_SHORT, S.quadIndices.data());
			offset += n;
			remaining -= n;
		}
...
void execDraw(const DrawSnapshot &snap)
{
...
void APIENTRY sh_Viewport(GLint x, GLint y, GLsizei w, GLsizei h) { es.Viewport(x, y, w, h); }
