...
#ifdef MC_WEBOS
	webos::log("[EXIT] game loop ended, leaving");
	webos::input::shutdown();
	// Leave immediately instead of returning through static destructors (SDL
	// window, audio device, worker threads): on the TV those can hang, and the
	// launcher then never sees the app go away.
	std::fflush(nullptr);
	_exit(0);
#endif
	return 0;
