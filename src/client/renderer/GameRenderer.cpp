...
#ifdef MC_WEBOS
	// On-screen keyboard for the remote: text screens only, never in the world.
	webos::osk::update(screen != nullptr && screen->wantsTextInput(), screen != nullptr && screen->textInputAutoOpen());
	webos::osk::setChat(screen != nullptr && screen->isChatScreen());
	if (screen != nullptr && mc.screen == screen)
		screen->renderOnScreenKeyboard();
#endif
}
