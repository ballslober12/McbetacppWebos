...
void HellCaveFeature::addRoom(int_t cx, int_t cz, std::array<ubyte_t, 16 * 16 * 128> &blocks, double x, double y, double z)
{
	addTunnel(cx, cz, blocks, x, y, z, 1.0f + random.nextFloat() * 6.0f, 0.0f, 0.0f, -1, -1, 0.5);
}

void HellCaveFeature::addTunnel(int_t cx, int_t cz, std::array<ubyte_t, 16 * 16 * 128> &blocks, double x, double y, double z, float paramFloat1, float paramFloat2, float paramFloat3, int paramInt3, int paramInt4, double paramDouble4)
{
	double d1 = (cx * 16 + 8);
	double d2 = (cz * 16 + 8);
	float f1 = 0.0f;
	...
	while (paramInt3 < paramInt4)
	{
		...
		if (is_room || random.nextInt(4) != 0)
		{
			...
			if (x >= d1 - 16.0 - d3 * 2.0 && z >= d2 - 16.0 - d3 * 2.0 && x <= d1 + 16.0 + d3 * 2.0 && z <= d2 + 16.0 + d3 * 2.0)
			{
				...
				// Abort if the carve region touches lava
				bool has_lava = false;
				for (int_t i3 = j; !has_lava && i3 < k; i3++) {
					for (int_t i4 = i1; !has_lava && i4 < i2; i4++) {
						for (int_t i5 = n + 1; !has_lava && i5 >= m - 1; i5--) {
							int_t i6 = (i3 * 16 + i4) * 128 + i5;
							if (i5 >= 0 && i5 < 128) {
								if (blocks[i6] == Tile::lava.id || blocks[i6] == Tile::calmLava.id)
									has_lava = true;
				...
				for (int_t i3 = j; i3 < k; i3++)
				{
					...
					for (int_t i4 = i1; i4 < i2; i4++)
					{
						...
						for (int_t i6 = n - 1; i6 >= m; i6--)
						{
							...
							{
								int_t t = blocks[i5];
								if (t == Tile::netherrack.id || t == Tile::dirt.id || t == Tile::grass.id)
									blocks[i5] = 0;
							}
							i5--;
						}
...
void HellCaveFeature::addFeature(Level &level, int_t xx, int_t zz, int_t x, int_t z, std::array<ubyte_t, 16 * 16 * 128> &blocks)
{
	(void)level;
	int_t i = random.nextInt(random.nextInt(random.nextInt(10) + 1) + 1);
