...
EntityThrownEgg::EntityThrownEgg(Level &level) : Entity(level)
{
	setSize(0.25f, 0.25f);
}

EntityThrownEgg::EntityThrownEgg(Level &level, double x, double y, double z) : EntityThrownEgg(level)
{
	setPos(x, y, z);
	heightOffset = 0.0f;
}

EntityThrownEgg::EntityThrownEgg(Level &level, Mob &owner) : EntityThrownEgg(level)
{
...
bool EntityThrownEgg::shouldRenderAtSqrDistance(double distance)
{
...
void EntityThrownEgg::shoot(double xd, double yd, double zd, float speed, float spread)
{
...
void EntityThrownEgg::setVelocity(double xd, double yd, double zd)
{
	this->xd = xd;
	this->yd = yd;
	this->zd = zd;
...
void EntityThrownEgg::tick()
{
	...
	if (hit.type != HitResult::Type::NONE)
	{
		...
		if (!level.isOnline && random.nextInt(8) == 0)
		{
			int_t spawnCount = random.nextInt(32) == 0 ? 4 : 1;
			for (int_t i = 0; i < spawnCount; ++i)
			{
				auto chicken = std::make_shared<Chicken>(level);
				chicken->absMoveTo(x, y, z, yRot, 0.0f);
...
void EntityThrownEgg::addAdditionalSaveData(CompoundTag &tag)
{
...
void EntityThrownEgg::readAdditionalSaveData(CompoundTag &tag)
{
